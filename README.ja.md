...
WaifuX は **Wallpaper Engine の公式製品ではありません**。Valve Corporation、Kristjan Skutta / Wallpaper Engine およびその関連会社との間に、**公式な提携、スポンサー関係、または所属関係は一切ありません**。アプリに統合された Wallpaper Engine シーン レンダリング機能は、**実験的な第三者互換実装**であり、ユーザーが既に所有している Workshop コンテンツまたはローカルファイルを、Rust + Metal（wgpu）で構築された自社ネイティブ GPU レンダリングパイプラインによってレンダリングするもので、個人的な学習・研究および相互運用性（interoperability）目的でのみ提供されています。
...
- ユーザーのデバイス構成、システムアップデート、GPU / グラフィックドライバーの互換性（特に Metal グラフィックススタック関連コンポーネント）に起因するレンダリング異常、クラッシュ、またはハードウェアの損傷
